...
export async function POST(req: NextRequest) {
  try {
    const { userText, provider = 'openai', model = 'gpt-4o-mini', history = [] } = await req.json()
    ...
    if (provider === 'openai') {
      ...
      let parsed: any
      try { parsed = JSON.parse(raw) } catch { parsed = { reply: raw, ipa: [], tips: '' } }
    ...
    if (provider === 'gemini') {
      ...
      let parsed: any
      try { parsed = JSON.parse(text) } catch { parsed = { reply: text, ipa: [], tips: '' } }
      return NextResponse.json(parsed)
    }
